...
#exp_name='SGD_rep0'
#exp_name='DP_rep0_e1'
#exp_name='DPSM_rep0_e01'
#args.figroot = exp_name+'/train/'
#args.figroot_test = exp_name+'/test/'

#args.filenames = args.figroot + '1.npz'
#args.gene = 'FASN'

raw_data_path = 'data/hist2tscript'

for exp_name in ['PPMLOmics_e5_rep0','PureSGD_e50_rep0']:
    #for args.gene in ['COX6C','GNAS','HLA-B','HSP90AB1','PABPC1','TFF3','FASN']:
    for args.gene in [ 'FASN']:
        args.figroot = '{}/pred_root/'.format(exp_name)

        print(exp_name,args.gene,args.figroot)
        ...
        #for subpath in [args.figroot,args.figroot_test]:
        for subpath in [args.figroot]:
            data = np.load(subpath + '10.npz')
            task = 'gene'
            ...
            for (patient, section) in sorted(set(ps)):
                ...
                image_filename = os.path.join(raw_data_path, "HE_{}_{}.tif".format(patient, section))
                ...
                def zscore(vec):
                    return ((vec - vec.mean(0)) / (3 * vec.std(0) + tol) + 0.5)
                fig = plt.figure(figsize=figsize)
                plt.scatter(pixel[mask, 0], pixel[mask, 1],c =value, cmap=plt.get_cmap('viridis'), s=10, linewidth=0, edgecolors="none")
                plt.gca().axis("off")
                plt.gca().set_aspect("equal")
                plt.xticks([])
                plt.yticks([])
